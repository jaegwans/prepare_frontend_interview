...
JWT 토큰을 저장할 때는 보안과 편의성 모두를 고려해야 합니다. 세션 스토리지, 로컬 스토리지, 쿠키는 각기 다른 장단점을 가지고 있어 특정 상황에 따라 다르게 사용할 수 있습니다. 다음은 각 저장 방식에 대한 설명과 장단점입니다:
...
### 브라우저에 도메인을 입력하면 어떤 일이 벌어지는지 설명해주세요.
...
브라우저의 렌더링 과정은 HTML을 파싱하여 DOM 트리를 생성, CSS를 파싱하여 CSSOM 트리를 생성, 이 두 트리를 결합하여 렌더 트리를 형성하는 단계를 포함합니다. 그 후, 렌더 트리의 노드들은 화면에 그려지는 위치와 크기를 계산하고(레이아웃,리플로우), 마지막으로 화면에 픽셀을 그려서(리페인팅) 사용자에게 보여줍니다.

#### 리플로우 (Reflow)

리플로우는 DOM의 구조적 변화가 일어나 렌더 트리를 다시 계산해야 할 때 발생합니다. 예를 들어, 요소의 크기 변경, 내용 추가/삭제, 페이지 초기 렌더링, 브라우저 창 크기 조정 등이 여기에 해당합니다. 리플로우는 페이지의 레이아웃을 재계산하므로 성능에 큰 영향을 줄 수 있습니다.

#### 리페인팅 (Repainting)

리페인팅은 요소의 시각적 변화가 발생했을 때, 예를 들어 색상이나 투명도 변경과 같은 상황에서 발생합니다. 리페인팅은 리플로우보다는 일반적으로 덜 비용이 많이 드는 작업입니다. 레이아웃 변경이 없이 시각적 스타일만 변경되는 경우에 발생합니다.
리플로우는 일반적으로 리페인팅을 동반하지만, 리페인팅이 반드시 리플로우를 필요로 하지는 않습니다. 효율적인 웹 개발을 위해서는 불필요한 리플로우와 리페인팅을 최소화하는 것이 중요합니다.

### osi 7계층에 대해 아시나요? 각각에 대해 설명해주세요.

OSI 7계층은 통신 과정을 7단계로 나눈 모델입니다:
물리계층(하드웨어 전송 기술),
데이터 링크 계층(네트워크 접속),
네트워크 계층(주소 결정과 경로 선택),
전송 계층(신뢰성 있는 통신),
세션 계층(연결 설정 및 유지),
표현 계층(데이터 형식 변환),
응용 계층(최종 사용자에 가까운 서비스).

### tcp/ip 4계층에 대해 알고 계신가요?

-   네트워크 엑세스 계층 (Link Layer)

이 계층은 물리적 네트워크 하드웨어와 직접적으로 연관되어 있습니다. 여기에는 물리적 장치와 프로토콜(예: Ethernet, Wi-Fi)이 포함되며, 데이터를 전기적 신호로 변환하고, 네트워크 상의 다른 장치와의 물리적 연결을 관리합니다.

-   인터넷 계층 (Internet Layer)

인터넷 계층은 패킷을 송수신하고, 다양한 네트워크를 거쳐 데이터를 전달합니다. 주요 프로토콜로는 IP(Internet Protocol)가 있으며, 이 계층은 데이터의 라우팅, 주소 지정, 패킷 분할 및 재조합 등을 담당합니다.

-   전송 계층 (Transport Layer)

전송 계층은 네트워크 연결의 종단 간 데이터 전송을 관리합니다. TCP(Transmission Control Protocol)와 UDP(User Datagram Protocol)가 이 계층의 대표적인 프로토콜입니다. TCP는 신뢰할 수 있는 연결 지향적 통신을 제공하고, UDP는 연결이 없는 더 빠른 전송을 제공합니다.
응용 계층 (Application Layer)

-   응용 계층은 사용자에게 서비스를 직접 제공하는 계층입니다. HTTP, FTP, SMTP와 같은 프로토콜이 이 계층에 포함되며, 이 계층에서는 사용자의 데이터와 프로토콜 데이터를 처리하고, 애플리케이션 간의 데이터 교환을 관리합니다.

### tcp/ip계층과 osi 7계층의 차이가 뭐죠

TCP/IP 모델은 실용적인 목적으로 개발되었으며, 인터넷의 기반이 되는 프로토콜 세트를 기반으로 합니다.
OSI 모델은 이론적인 접근을 기반으로 하며, 네트워크 통신 전반의 추상적인 표준을 제공합니다.

### tcp udp

-   tcp : 연결회선 방식이며 안정성있음 . 순서 보장, 3way 4way,
-   udp : 비연결회선 비안정성. 데이터그램 방식 , 영상스트리밍 등

### 객체지향의 특징에 대해 알고계신가요? 추상화가 뭐죠?

객체지향 프로그래밍은 캡슐화, 상속, 다형성과 같은 특징을 갖습니다. 추상화는 복잡한 실제 세계를 단순한 모델로 표현하는 것을 말합니다. 이는 필요한 정보만을 간추려 프로그램에 적용하는 과정입니다.

### 객체지향의 SOILD원칙에 대해 알고계신가요?
...
해쉬 함수를 통해 키를 해시값으로 변환하고 이 해시값을 인덱스로 사용하여 데이터를 저장하는 자료구조

일반적으로 시간복잡도는 O(1) 이며 추가, 삭제 검색등이 요이합니다.
해시 함수가 같은 키를 반환할 수 있어 이 경우체이닝이나 오픈 어드레싱 같은 기법을 사용합니다.

### 레드블렉트리

최악의 경우에도 O(logN) 유지
레드-블랙 트리는 효율적인 검색, 삽입, 삭제 연산을 지원하는 자가 균형 이진 탐색 트리입니다. 이 트리의 구조는 특정 규칙을 따라 노드들이 레드(빨간색) 또는 블랙(검은색)으로 칠해져 있으며, 이 색상은 트리의 균형을 유지하는 데 중요한 역할을 합니다. 레드-블랙 트리의 주요 규칙은 다음과 같습니다:

모든 노드는 레드 또는 블랙입니다.
루트 노드는 블랙입니다.
모든 리프 노드(NIL 노드)는 블랙입니다.
레드 노드의 자식 노드들은 모두 블랙입니다 (즉, 레드 노드는 연속될 수 없습니다).
각 노드로부터 해당 노드의 모든 리프 노드까지의 경로에 있는 블랙 노드의 수는 모두 같습니다.
노드가 삽입되거나 삭제될 때, 이 규칙들이 위배되지 않도록 트리는 재조정되어야 합니다. 이를 위해 트리는 회전과 색상 변경을 통해 구조를 조정합니다. 삽입이나 삭제로 인해 규칙 위반이 발생할 경우, 다음과 같은 조치를 취할 수 있습니다:

색상 변경: 특정 노드의 색상을 변경하여 규칙 위배를 해결합니다.
회전: 트리의 구조를 변경하지 않고 트리의 균형을 맞추기 위해 노드들의 위치를 회전시킵니다. 회전은 LL, RR, LR, RL 회전의 네 가지 유형이 있습니다

엉클노드의 색이 블렉이다 -> 리스트럭처링 후 리컬러링
// 색이 레드이다 -> 리컬러링(엉클 부모 블랙.자식 조상 레드) -> (만약 조상이 루트면) -> 조상만 블랙으로
